...
import numpy as np
from datagen import To_tensor_4
...
ckpt_hin_1 = '/DATA/ocr_team_2/VTNet/sample_test_data_1nov/train_step-250000.model'
...
size = (128, 64)
def infer(i_s, i_t, b_g, size, model, path):
    ...
    b_g = io.imread(b_g)
    b_g = cv2.resize(b_g, size)

    i_t, i_s, b_g = tmfr([i_t, i_s, b_g])

    i_t = i_t.unsqueeze(0).cuda()
    i_s = i_s.unsqueeze(0).cuda()
    b_g = b_g.unsqueeze(0).cuda()

    with torch.no_grad():
      o_sk, o_t, o_b, o_f = model(i_t, i_s, b_g (i_t.shape[2], i_t.shape[3]))
...
i_s = '/DATA/ocr_team_2/VTNet/sample_test_data_1nov/i_s/191_0.jpg'
i_t = '/DATA/ocr_team_2/VTNet/sample_test_data_1nov/b_g/191_0.jpg'
b_g = '/DATA/ocr_team_2/VTNet/sample_test_data_1nov/i_t/191_0.png'
size = (128, 64)
path = '/DATA/ocr_team_2/VTNet/sample_test_data_1nov/result/test.png'
infer(i_s, i_t, b_g, size, G_hin, path)
...
#   demo.launch(share=True)
  ...
  # data_root = './1k_translations/visualizer/static/'
  ...
  #   i_t_path = os.path.join(data_root, 'i_t_2', f'{idx}.png')

  #   out_path_1 = os.path.join(data_root, 'out_m3', f'{idx}.png')
